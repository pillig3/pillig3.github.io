...
function drawBoard(): void {
  ...
  const scoreStartPos = new Pair(boardLeft + Math.floor(cellHeight * (NUM_COLUMNS + 1)), TOP_MARGIN + Math.floor(6 * cellHeight));
 ...
 */
function drawSinglePieceToTheSide(startPosition: Pair, piece: string, edgeColor: string, pieceColor: string): void {
  // Background
  ...
  forEachElementOfShapeGrid(pieces[piece].shape, (col, row, element) => {
    if (element) {
      const start = new Pair(startPosition.x + (col + 1) * cellHeight, startPosition.y + (row + 1) * cellHeight);
 ...
 */
function drawBorderedRectangle(xStart: number, yStart: number, width: number, height: number, borderColor: string, fillColor: string): void {
  // Border
  ctx.lineWidth = 1;
 ...
 */
function forEachElementOfShapeGrid(shape: boolean[][], callback: (col: number, row: number, element: boolean) => void): void {
  for (let col = 0; col < shape.length; col++) {
    for (let row = 0; row < shape[0].length; row++) {
