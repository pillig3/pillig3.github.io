...
var cols = 4;
var playerCanMove = 1;
function gebi(x: string): HTMLElement { return document.getElementById(x); }

/* when player clicks tile (c, r) */
function playerMove(c: number, r: number) {
  if (playerCanMove) {
    playerCanMove = 0;
    monch(c, r);
    makeLoadingImg();
...
function monch(c: number, r: number) {
...
function takeABite(pos: number[], c: number, r: number): number[] {
  const pos1 = pos.slice();
  for (let col = c - 1; col < pos1.length; col++) {
...
function isLosingPosition(pos: number[]) {
...
function setupChomp() {
  rows = parseInt((gebi("chompRowNumber") as HTMLButtonElement).value);
  cols = parseInt((gebi("chompColumnNumber") as HTMLButtonElement).value);
...
function changeBoardOnscreen(list: number[]) {
  let originNeedsPlacing = 1;
  let rowStringList = new Array(rows).fill("<tr>");
  let bottomCells = 0;
