import {
  ...
  sine, cosine, tangent, sinh, cosh, tetrate, iterate, ComplexNumber, sqrt
} from './complexNumbers';
import { functify } from './functify';
import { Token, TokenType } from "./tokenize";
export { Node, NodeChild, ComplexFunction, parse };
  ...
  if (isNamedFunction(tokenOne.text)) {
    ...
    if (tokens[1].text === '(') {
      ...
      if (endInd === tokens.length - 1) {
        ...
        return new Node(func as ComplexFunction, args.map((x) => parseRec(x, vars)));
      } else {
        // There are tokens past the arguments - we're multiplying with the next part
        children.push(new Node(func as ComplexFunction, args.map((x) => parseRec(x, vars))));
      ...
      if (i > 0) {
        ...
        if (prevToken.type !== TokenType.Function) {
          ...
          if (!(['+', '-'].indexOf(strF) >= 0)  // +/- are lower priority
            && ([TokenType.Number, TokenType.String].indexOf(prevToken.type) >= 0 || prevToken.text === ')')
            && ([TokenType.Number, TokenType.String].indexOf(token.type) >= 0 || token.text === '(')
